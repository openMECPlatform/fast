...
import edu.wisc.cs.wisdom.sdmbn.json.ExtSynMessage;
import edu.wisc.cs.wisdom.sdmbn.operation.Operation;
import edu.wisc.cs.wisdom.sdmbn.json.DestinationInfoMessage;  // for FAST
...
public class StateChannel extends BaseChannel 
{
	...
	protected void processMessage(String line) throws MessageException
	{
		...
		if (msg instanceof ExtSynMessage)
		{
			ExtSynMessage syn = (ExtSynMessage)msg;
			log.info("[SDMBN] - SYN from {}.{}", syn.host, syn.pid);
			log.debug("FAST: state info {} {}", syn.stateIp, syn.statePort);
			this.host = syn.host;
			this.pid = syn.pid;
//			sdmbnManager.channelConnected(this);
			sdmbnManager.channelConnected(this, syn.stateIp, syn.statePort);
	...
	public void sendMessage(Message msg) throws MessageException 
	{
		...
		else if (msg instanceof DestinationInfoMessage)
		{
//			log.debug("FAST: I must be here!!!!");
			json = gson.toJson((DestinationInfoMessage)msg);
		}
		else
		{ throw new MessageException("Unexpected message type: "+msg.type, msg); }
