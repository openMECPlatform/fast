...
//import edu.wisc.cs.wisdom.sdmbn.json.DestinationInfoMessage; // for test only
...
//	public void stateForwarding(IFloodlightProviderService floodlightProvider, IRoutingService rtEngine)
//	{
//		DirectStateForwarding dirStateFw = new DirectStateForwarding(floodlightProvider, rtEngine, this.src);
//		dirStateFw.forwardToMiddlebox(this.key, this.dst);
//	}

	@Override
    public int execute()
    {
    	// prepare the state forwarding rules
//		log.debug("FAST: the object is {}", this.manager.routingEngine);
	...
	private int issueGet()
	{
		...
		if (Scope.PERFLOW == this.scope || Scope.PF_MF == this.scope)
		{
    		...
    		try
    		{
				this.moveStartNew = System.currentTimeMillis();
				log.debug("FAST: Official Move Start ({})", this.getId());
    			this.src.getStateChannel().sendMessage(getPerflow);

    		}    // getPerFlow message is sent from here
    		catch(MessageException e)
    		{ return -1; }

//			DestinationInfoMessage destInfo = new DestinationInfoMessage(this.getId(), this.key, this.dst.stateIp, this.dst.statePort);

//			log.debug("FAST: what's going on: {}", destInfo instanceof DestinationInfoMessage);

//			try
//				{ this.src.getStateChannel().sendMessage(destInfo); }    // getPerFlow message is sent from here
//			catch(MessageException e)
//			{
...
//			}
    ...
    private void receiveStateMessage(StateMessage msg)
    {
		...
		// If hashkey exists, simply store msg. Otherwise, create new chunk
	...
	{	
		...
		else if (from == this.src)
		{
			...
			log.debug("FAST: get host", this.src.getHost());
    ...
    public void receiveGetPerflowAck(GetPerflowAckMessage msg) 
    {
        ...
        {
                //log.debug("Tung: I am here for no optimization check");
        ...
        { checkIfStateMoved();
        //log.debug("Tung: I am here to check state moved");
        }
    }
...
//		boolean completed = false;
		...
		{
			case PERFLOW:
				log.debug("Tung: number of putAck ({})", this.putPerflowAcks);
				log.debug("Tung: number of getPerFlow count ({})", this.getPerflowCount);
		...
		if (completed)
		{
			...
			long stateMoveEnd = System.currentTimeMillis();
			long stateMoveTime = stateMoveEnd - this.moveStart;
			log.info(String.format("[MOVE_TIME] state_elapse=%d, start=%d, end=%d",
					stateMoveTime, this.moveStart, stateMoveEnd));

			log.info(String.format("[OFFICIAL_MOVE_TIME] state_elapse=%d, start=%d, end=%d",
					stateMoveEnd - this.moveStartNew, this.moveStartNew, stateMoveEnd));
	...
	public void receiveEventsAck(EventsAckMessage msg, Middlebox from)
	{
		...
		else
		{
			...
			long moveEndFirst = System.currentTimeMillis();
			long moveTimeFirst = moveEndFirst - this.moveStart;
			log.info(String.format("[MOVE_TIME] elapse=%d, start=%d, end=%d",
					moveTimeFirst, this.moveStart, moveEndFirst));
	...
	public void receivePacket(byte[] packet, IOFSwitch inSwitch, short inPort) 
	{
		...
		{
//			log.debug("FAST: Config port and received port: {} {}", this.inPort, inPort);
//			log.debug("FAST: Config sw and received sw: {} {}", this.src.getSwitch(), inSwitch);
//			log.debug(String.format("[Move %d] Received packet from wrong switch port", this.getId()));
			return; 
		}
		
		this.packetCount++;  // This shows number of inflight packets.
